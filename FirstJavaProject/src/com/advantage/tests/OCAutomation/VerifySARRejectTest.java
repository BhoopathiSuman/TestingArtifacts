...
public class VerifySARRejectTest extends TestTemplate {
	...
	public static void runVerifySARReject(LoginDetails details, List<CreateNewSARDetails> createNewSARtDtl) throws InterruptedException, ParseException
	{
		...
		try{
			for(CreateNewSARDetails createlead:createNewSARtDtl){			
				...
				SARDtlPage.EnterCommentToSAR("Rejected");
				SARDtlPage.clickActionButton("Reject");
				if(SARDtlPage.verifyStatus("LOA- Rejected")==true){
					Report.logPass("SAR status changed to LOA-Rejected after submitting for approval");
				}
				else{
					Report.logError("SAR status has not changed to LOA-Rejected after submitting for approval");
				}
				homePage.clickLogout();				
			}
			...
			Report.logError("Verify SAR Rejected Test failed.", ex);
		}
	}
}
