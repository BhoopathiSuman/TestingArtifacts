...
public class CreateNewSAR extends TestTemplate {
	
	private static final String CreateNewSAR = "runCreateNewSAR";
	...
	public static void runCreateNewSAR(LoginDetails details, List<CreateNewSARDetails> createNewSARtDtl) throws InterruptedException, ParseException
	{
		/* Test case initiation */
		
		Report.logTitle(Framework.getNewLine() + "Method " + CreateNewSAR + " executing ..."
		...
		try{
			for(CreateNewSARDetails createlead:createNewSARtDtl){			
				...
				String sSAR_ID= SARDtlPage.getSARID();
				String sSAR_Status = SARDtlPage.getSARStatus();
				
				if((sSAR_ID.isEmpty()==false) && (sSAR_Status.isEmpty()==false)){
					Report.logPass("SAR has been created successfully with status as - " +  sSAR_Status + " and its ID is - " + sSAR_ID);
				}
				else{
				...
				sSAR_Status=null;
				sSAR_Status = SARDtlPage.getSARStatus();
				
				if(sSAR_Status.contains("QA")){
					Report.logPass("SAR has been submitted to Pricing Analyst and its status changed to - " + sSAR_Status);
				}
				else{
			...
			Report.logError("Create New SAR Scenario failed.", ex);
		}
	}
}
