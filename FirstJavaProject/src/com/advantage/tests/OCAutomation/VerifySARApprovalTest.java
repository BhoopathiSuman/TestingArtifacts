...
public class VerifySARApprovalTest extends TestTemplate {
	
	private static final String VerifySARApproval = "runVerifySARApproval";
	...
	public static void runVerifySARApproval(LoginDetails details, List<CreateNewSARDetails> createNewSARtDtl) throws InterruptedException, ParseException
	{
		/* Test case initiation */
		
		Report.logTitle(Framework.getNewLine() + "Method " + VerifySARApproval + " executing ..."
		...
		try{
			for(CreateNewSARDetails createlead:createNewSARtDtl){			
				...
				//String sSAR_ID ="SAR-0000003452";
				homePage.clickSARLink(sSAR_ID);
				SARDtlPage.clickApproveRejectLink();
				SARDtlPage.EnterCommentToSAR("Approved");
				SARDtlPage.clickActionButton("Approve");
				if(SARDtlPage.verifyStatus("LOA-Approved")==true){
					Report.logPass("SAR status changed to LOA-Approved after submitting for approval");
				}
				else{
					Report.logError("SAR status has not changed to LOA-Approved after submitting for approval");
				}
				homePage.clickLogout();	
				homePage.verifyLoginPage();				
				loginPage.loginAs(ConfigInfo.getPAsUserName(), ConfigInfo.getsPAPassword());				
				homePage.clickSARLink(sSAR_ID);
				SARDtlPage.clickClosedButton();
				if(SARDtlPage.verifyStatus("Closed")==true){
					Report.logPass("SAR status changed to Closed");
				}
				else{
					Report.logError("SAR status has not changed to Closed");
				}
				homePage.clickLogout();
			}
			...
			Report.logError("Verify SAR Approval Test failed.", ex);
		}
	}
}
